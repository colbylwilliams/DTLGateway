...
print('')
print('Deploying function app source')

funcapp_deploymentj = subprocess.run([
    'az', 'functionapp', 'deployment', 'source', 'config', '-n', funcapp_name, '-g', rg, '--subscription', sub,
    '-u', 'https://github.com/colbylwilliams/DTLGateway',
    '--branch', 'main',
    '--manual-integration'
], stdout=subprocess.PIPE, universal_newlines=True).stdout

try:
    funcapp_deployment = json.loads(funcapp_deploymentj)
except json.decoder.JSONDecodeError:
    raise ChildProcessError(
        'Failed to deploy function app source: {}'.format(funcapp_deploymentj))


print('')
print('Generating function key')

funcapp_tokenj = subprocess.run([
...
try:
    funcapp_token = json.loads(funcapp_tokenj)
except json.decoder.JSONDecodeError:
    raise ChildProcessError('Failed to generate function key: {}'.format(funcapp_tokenj))
...
print('')
print('Gateway public IP address: {}'.format(ip['ipAddress']))
print('Gateway token secret: {}'.format(funcapp_token['value']))
